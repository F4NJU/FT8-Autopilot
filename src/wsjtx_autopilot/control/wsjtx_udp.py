...
from wsjtx_autopilot.wsjtx.models import ClearPacket, WsjtxPacket
...
class WsjtxUdpControl:
    ...
    def execute(self, action: IntendedAction, now: datetime) -> ActionOutcome:
        ...
        if self._max_actions is not None and self._actions_used >= self._max_actions:
            self._armed = False
            LOGGER.warning(
                "[CONTROL] actions=%d/%d - new initiations disarmed",
                self._actions_used,
                self._max_actions,
            )
    ...
    def poll(self) -> None:
        if self._armed and self._kill_switch_file is not None and self._kill_switch_file.exists():
            self.disarm(f"kill switch file detected: {self._kill_switch_file}")

    def disarm(self, reason: str = "software kill switch") -> None:
        if self._armed:
            self._armed = False
            LOGGER.warning("[CONTROL] DISARMED reason=%s", reason)
    ...
    def set_tx_df(self, action: IntendedAction, tx_df: int, now: datetime) -> bool:
        """Request patched WSJT-X to change Tx DF without consuming an action."""
        rejection = self._validate(action, now)
        if rejection is not None:
            LOGGER.warning("[CONTROL] SetTxDF rejected station=%s reason=%s", action.station, rejection)
            return False
        decode = action.original_decode
        assert decode is not None and decode.source_endpoint is not None
        try:
            datagram = serialize_set_tx_df(decode.schema, decode.instance_id, tx_df)
            sent = self._transport.sendto(datagram, decode.source_endpoint)
            if sent != len(datagram):
                raise OSError(f"partial UDP send ({sent}/{len(datagram)} bytes)")
        except (OSError, ProtocolError) as exc:
